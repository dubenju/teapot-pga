...
public class AnalyzerHtml extends AnalyzerBinary {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.matches("^[\\s&&[^\\n]]*$")) {
                    ...
                    result.countNormalLines();
                    commentHtml = true;
                } else if (commentHtml == true && !line.endsWith("-->")) {
                    // 为多行注释中的一行（不是开头和结尾）
                    result.countCommentLines();
                } else if (commentHtml == true && line.endsWith("-->")) {
                    // 为多行注释的结束行
                    result.countCommentLines();
                    commentHtml = false;
                } else if (commentHtml == true && line.indexOf("-->") >= 0) {
                    // 为多行注释的结束行
                    result.countNormalLines();
                    commentHtml = false;
